...
namespace ShopToys.Areas.Admin.Controllers
{
    ...
    public class AdminRolesController : Controller
    {
        ...
        public AdminRolesController(ManagerShopPRN211Context context, INotyfService notyfService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index()
        {
              return _context.Roles != null ? 
                          ...
                          Problem("Entity set 'ManagerShopPRN211Context.Roles'  is null.");
        }
        ...
        public async Task<IActionResult> Create([Bind("RoleId,RoleName")] Role role)
        {
            if (ModelState.IsValid)
            {
                ...
                _notyfService.Success("Add role successfully.");
        ...
        public async Task<IActionResult> Edit(int id, [Bind("RoleId,RoleName")] Role role)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    await _context.SaveChangesAsync();
                    _notyfService.Success("Edit role successfully.");
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!RoleExists(role.RoleId))
                    {
                        _notyfService.Success("Some thing wrong please try again.");
                        return NotFound();
                    }
                    else
                    {
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                return Problem("Entity set 'ManagerShopPRN211Context.Roles'  is null.");
            }
            var role = await _context.Roles.FindAsync(id);
            if (role != null)
            {
